...
function runTests() {
  ...
  if (!testCmd) {
    console.log('[power-workflow] No test framework detected');
    console.log('Supported: npm/yarn/pnpm, pytest, cargo, go, dotnet, make');
    process.exit(1);
  }

  console.log(`[power-workflow] Running: ${testCmd}`);
  console.log('---');
  ...
  try {
    ...
    if (result.status === 0) {
      console.log('---');
      console.log('[power-workflow] Tests PASSED');
      process.exit(0);
    } else {
      console.log('---');
      console.log('[power-workflow] Tests FAILED');
      process.exit(result.status || 1);
    }
  } catch (error) {
    console.error('[power-workflow] Error running tests:', error.message);
    process.exit(1);
  }
}
